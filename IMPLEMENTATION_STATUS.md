...
**Dernière mise à jour :** 13 août 2026 (écart produit — **export concaténé des zones remplies** : l'export ne porte plus sur un segment unique mais **concatène toutes les zones de cases prêtes**, les trous étant supprimés du montage — vidéo ET musique ; voir la section dédiée, **relecture adversariale de cet écart incluse — menée le 13 août, 25 défauts, corrections listées**. Cet écart REMPLACE celui du 11 août 2026 « export du segment rempli ». Précédemment : revue finale du Jalon 12 — correctifs §39/§60/§61/§62/§64 et état honnête des livrables §87)
...
- **`AssemblyView`** — « Export » (dock §36) et « Prévisualiser le montage » (§47.2, zone basse) sont actifs dès qu'**au moins une case est prête**, plus seulement quand la première case est prête ni quand un segment unique existe. `AssemblyViewLogic.isExportEnabled(slots:)` passe par `readyTimeline(slots:)` ; la variante « items » est un simple `contains { $0.state == .ready }` (O(N), sans allocation). `exportedSegmentPositions` (plage unique) est **remplacé** par `exportedZonePositions` / `exportedZoneIndexes` (**toutes** les zones). Le titre de la feuille d'aperçu nomme ce qui est lu (« Montage — plans 28 à 50 », « Montage — 19 plans en 2 zones »). **Depuis la relecture adversariale du 13 août** : le nombre de plans annoncé vient d'une source UNIQUE (`AssemblyViewLogic.exportedSlotCount(items:)`, pendant de `ReadyTimeline.slotCount`) et non plus de la largeur des plages d'index ; le hint du bouton « Export » désactivé dépend de la CAUSE (§64) — « Remplissez au moins une case », « attendez la fin du téléchargement » ou « remplacez la vidéo indisponible ou trop courte » (`AssemblyViewLogic.nothingReadyCause(items:)` → `ExportSummaryLogic.NothingReadyCause`, table de messages unique).
- **`AssemblyMiniTimelineView`** (§35.3 « limite d'export partiel ») — le crochet **unique** est remplacé par **un crochet PAR ZONE** : même forme (liséré fin fermé par un montant vertical à chaque bout), simplement **répétée**, avec une séparation d'un point garantie entre deux crochets voisins. L'utilisateur voit d'un coup d'œil **les morceaux qui partiront** et **les trous qui seront supprimés**. Raison : marquer une seule zone en cacherait d'autres qui partent pourtant à l'export. Trois repères, **trois formes distinctes** et trois emplacements (§39, « ne pas dépendre uniquement de la couleur ») : position courante = barre verticale pleine hauteur, fenêtre du carrousel = soulignement plein **sous** la piste, zones exportées = crochets **au-dessus**. La valeur VoiceOver annonce « Plan 4 sur 10, **19 plans exportables en 2 zones** » (et garde « plans 28 à 50 exportables » quand il n'y a qu'une zone), avec le compte faisant autorité (`exportedSlotCount`). **Correctif de la relecture adversariale du 13 août** : les crochets ne DÉRIVENT plus. La largeur minimale (3 pt) et le décalage anti-chevauchement s'ACCUMULAIENT de zone en zone — au-delà d'une centaine de cases, un crochet pouvait être dessiné entièrement à côté de sa zone. Chaque crochet est désormais posé sur SA plage : la largeur minimale est **centrée** sur la plage réelle (et glisse à l'intérieur de la vue si elle déborde d'un bord), la séparation entre voisins **rétrécit** le crochet courant au lieu de le déplacer, et elle est abandonnée s'il n'y a plus la place — l'erreur de position est bornée par 1,5 pt, quel que soit le nombre de zones.
- **`ExportSummaryView`** (§56) — le résumé dit ce qui part **vraiment** : nombre total de plans + durée (ligne §56 verbatim), puis, **quand l'export est partiel**, `exportedPlansLabel` — « Plans 28 à 50 » pour **une** zone (formulation conservée), « **19 plans en 2 zones** » à partir de deux — et, en petit, les plages exactes `zoneRangesLabel` (« 28–35, 40–50 »). La **durée** affichée est la **somme des durées des cases exportées** (les trous ne comptent pas). **Mention honnête ajoutée** sous le résumé dès qu'il y a plusieurs zones : `concatenationNotice` — « Les zones sont mises bout à bout : la musique passe directement d'une zone à la suivante. » (sobre, jamais alarmante : c'est le comportement demandé). `partialExportNotice` ne promet plus que « rien n'est déplacé » — il dit ce qui est ignoré et que **le projet** reste intact. Nouveaux helpers purs testés : `exportedPlansLabel(zones:slotCount:)`, `plansAndZonesLabel(slotCount:zoneCount:)`, `zoneCountLabel(_:)`, `zoneRangesLabel(zones:)`, `spokenZoneRanges(zones:)` (§39 : « plans 28 à 35, puis plans 40 à 50 » — le tiret ne se lit pas), `concatenationNotice`. **Depuis la relecture adversariale du 13 août** : (a) l'instantané ANNONCÉ est celui qui est encodé (`ExportActor.startExport(projectID:snapshot:)`) — l'écran ne fige plus un résumé pendant que l'acteur relit le projet ; (b) un export **tronqué** §66 n'est plus annoncé comme un succès plein : `finish()` compare l'annonce figée à l'appui (`announced`) et les mesures du fichier écrit (`ExportOutcome.slotCount`/`duration`/`masterProfile`) et bascule la phase `ready` sur l'origine `.truncated` — « Export incomplet », ce qui a été écrit, pourquoi, et quoi faire ; (c) le compte de plans des libellés est celui du domaine (`ReadyTimeline.slotCount`), écran ET VoiceOver ; (d) §64 : le geste proposé quand rien n'est prêt dépend de la cause (remplir / attendre / remplacer) ; (e) §57 : la vérification d'espace de la vue applique la MÊME règle que l'exportateur (`requiredBytesIncludingPhotosCopy` — encodage **et** copie Photos), au lieu de l'estimation nue.
- **`PreviewPlayerView`** (§47, §64) — les textes parlent des **zones exportées** : le message « aucun plan prêt » nomme le geste qui dépend de la CAUSE (§64 — remplir, attendre ou remplacer, table unique d'`ExportSummaryLogic`), hint d'export « les zones remplies que vous venez de voir », et pour `PreviewError.incompletePrefix` « **Un plan de ce montage** n'est pas encore prêt ». L'aperçu est le seul endroit où l'utilisateur **entend** les jonctions avant d'exporter.
- **`ClipPickerView`** (§46) — **correctif de la relecture adversariale du 13 août** : la phase « Montage complet » signifie « plus aucune case VIDE », pas « tout est prêt » (une case peut être remplie et en téléchargement §44, indisponible §64 ou trop courte §43). L'aperçu qu'elle propose n'est donc plus intitulé « Montage complet » ni décrit comme « Lit le montage depuis le début » : il annonce « **Montage — zones prêtes** » et « Lit les zones prêtes du montage, mises bout à bout ». Le titre §46 de l'écran, lui, reste verbatim.
...
- `Tests/Unit/AssemblyViewLogicTests.swift` — deux verdicts ont **changé** : `testExportEnabledWhenFirstSlotIsEmptyButALaterSlotIsReady` (une première case vide n'empêche rien) et surtout `testEveryZoneIsExportedNotOnlyTheFirst` (avant : seule la première zone comptait). Ajouts : `testExportEnabledWithAnEmptyFirstSlotAndTwoZones` (première case vide **et** deux zones), `testZonesFollowTheShapeOfTheRequestExample` (index 28..35 + 40..50 → 19 plans en 2 zones), zone d'un seul plan, chaque état non prêt **scinde** les zones au lieu de borner le montage, traduction position → index de case, énoncé VoiceOver `spokenExportedZones` (une zone, plusieurs zones, index défensifs). **Ajouts de la relecture adversariale du 13 août** : `testInterfaceZonesMatchDomainRunsOnEveryArrangement` — sur une MATRICE d'arrangements, les zones de l'interface (`exportedZoneIndexes`) et les runs du domaine (`readyTimeline(slots:).runs`) doivent avoir les mêmes bornes, le même nombre de plans par zone et le même total (les deux découpages étaient écrits séparément et rien ne les comparait) ; même vérification avec des index ne commençant pas à zéro ; `exportedSlotCount` (compte faisant autorité ; **second passage** : un index manquant SCINDE désormais la zone, comme dans le domaine, donc la largeur d'une plage vaut de nouveau son nombre de plans) ; `nothingReadyCause` (§64) et son ACCORD avec la dérivation du résumé §56.
- `Tests/Unit/ExportSummaryLogicTests.swift` — formatage **une** zone (plage conservée, singulier « Plan 28 »), **deux** zones et plus (« 19 plans en 2 zones » + « 28–35, 40–50 » + version parlée), **zone d'un seul plan**, **aucune** zone (helpers `nil`), accord singulier de `zoneCountLabel`, et deux tests de FORMULATION : `partialExportNotice` ne promet plus que rien n'est déplacé, `concatenationNotice` est honnête sur le saut de musique **sans** vocabulaire alarmant. **Ajouts de la relecture adversariale du 13 août** : §66 export tronqué (`divergence` — aucun écart quand le fichier correspond, détection sur le nombre de plans et la durée, détection d'un profil §52 différent, jamais d'écart inventé quand le profil produit est inconnu), `truncatedExportMessage` (ce qui a été écrit, pourquoi, quoi faire ; aucune comparaison « 19 plans au lieu de 19 plans » quand seul le profil change), `readyTitle/readyMessage/readySystemImage(origin:)` (trois origines, trois discours, trois icônes), compte faisant autorité passé à `exportedPlansLabel`, et §64 `nothingReadyCause`/`nothingReadyHint` (quatre causes, quatre gestes distincts).
...
### Relecture adversariale de cet écart (13 août 2026) — ELLE A BIEN EU LIEU

Elle **avait été oubliée** : l'écart produit du 13 août avait été livré sans la passe de relecture que tout changement de cette ampleur exige, et cette section l'affirmait implicitement en ne mentionnant que la vérification Mac. C'est corrigé — la relecture a eu lieu le **13 août 2026**, sur le changement complet (domaine, services, interface, tests).

**Ce qu'elle a trouvé — 25 défauts, dont :**

- **1 bloqueur — le cœur du changement n'était pas testé.** Le modèle temporel de la concaténation (position de composition d'un run, insertion audio par run, invariants « aucun trou entre deux runs », « aucune case déplacée à l'intérieur d'un run », « durée totale = somme des durées des cases prêtes ») ne l'était que par ses conséquences visibles. Une régression de placement serait passée silencieusement.
- **Divergences annonce / production.** §66 : un export **tronqué** (rush devenu indisponible pendant l'encodage) était annoncé comme un succès plein — « Votre montage est exporté. » — alors que l'issue portait le vrai nombre de plans et la vraie durée. §52 : le profil maître annoncé avant export portait sur la timeline complète, alors que le fichier livré pouvait avoir été encodé avec un autre clip maître. Et le montage **annoncé** n'était pas le montage **encodé** : la vue figeait son résumé à l'ouverture pendant que l'acteur relisait un instantané frais à l'appui.
- **Invariants de run.** Deux découpages en zones coexistaient — le domaine (`readyTimeline`, qui pilote l'export) et l'interface (`AssemblyViewLogic.exportedZonePositions`, qui dessine et nomme) — sans aucun test pour les comparer. Le nombre de plans, lui, était affiché deux fois sur le même écran depuis deux calculs différents (compte du domaine d'un côté, somme des largeurs d'intervalles d'index de l'autre).

**Côté domaine et services** (agent Cœur, en parallèle) : le modèle temporel de la concaténation est repris avec ses invariants et ses tests (`ReadyTimelineTests`, `ExportPlanningTests`), l'export accepte l'instantané VALIDÉ (`ExportActor.startExport(projectID:snapshot:)`, sans relecture de la base) et l'issue décrit le fichier RÉELLEMENT écrit — `ExportResult.masterProfile` / `ExportOutcome.masterProfile` (`nil` = inconnu, jamais un profil par défaut).

**Ce qui a été corrigé côté interface** (détail dans les puces « Conséquences dans l'interface » ci-dessus) : instantané annoncé transmis à l'export (`startExport(projectID:snapshot:)`) ; export tronqué annoncé comme tel, avec ce qui a été écrit, la cause et le geste à faire ; profil §52 réellement utilisé comparé à celui annoncé (`ExportOutcome.masterProfile`) ; compte de plans unique (écran et VoiceOver) ; §64 le geste proposé dépend de la cause (remplir / attendre / remplacer) ; §57 même seuil d'espace disque que l'exportateur (copie Photos comprise) ; §35.3 crochets de zones qui ne dérivent plus ; §46 l'aperçu de la photothèque ne promet plus « le montage depuis le début ». Un test compare désormais, sur une matrice d'arrangements, les zones de l'interface et les runs du domaine.

### Second passage de relecture (13 aoû 2026) — 7 défauts survivants

La passe précédente avait laissé sept défauts, corrigés ici :

1. **Les deux découpages en zones divergeaient toujours** (findings 5 + 23). `AssemblyViewLogic.exportedZonePositions` ne fermait une zone que sur un changement d'ÉTAT, alors que `readyTimeline(slots:)` ferme aussi un run sur toute **discontinuité** (index sauté, cases non jointives). Sur des cases d'index 28, 29, 31, l'écran annonçait « 4 plans en 2 zones » là où l'export produit **3** zones et 3 coupes musicales. Le test d'équivalence était aveugle : sa matrice ne fabriquait que des index contigus et des cases jointives. L'interface applique désormais les trois mêmes règles de fermeture, et la matrice porte les deux discontinuités (`SlotSpec.readyAfterMissingIndex`, `.readyAfterTimeGap`).
2. **Les sources d'aperçu matérialisées n'étaient jamais purgées** (régression du finding 10). L'aperçu écrit ses ré-encodages de rushs recomposés dans `previews/`, que rien ne vidait — coût disque hors §57, et un rush **retouché** dans Photos aurait été rejoué éternellement dans sa version périmée pendant que l'export encodait la nouvelle. `ProjectFileStore.clearPreviewIntermediates(projectID:)` est appelé par `PreviewCache.invalidateAll(projectID:)`, et le nom du fichier intermédiaire porte désormais la **date de modification** de l'asset.
3. **Convention de numérotation** (finding 13). L'exemple « 28…35 / 40…50 » était encodé à deux conventions incompatibles. Une seule est déclarée en tête d'`ExportModels.swift` : le CODE est en index 0-based, l'INTERFACE affiche +1, et l'exemple de la demande est écrit dans la numérotation de l'utilisateur (**plans 28 à 35 et 40 à 50**, soit les index 27…34 et 39…49). Les fixtures du domaine gardent leurs index 28…35 / 40…50 — elles reproduisent la FORME de l'exemple, et leurs en-têtes le disent (décaler leurs dizaines de littéraux calculés à la main aurait coûté plus qu'il ne rapporte).
4. **API morte supprimée** (finding 25) : `MediaLibraryActor.videoAssetURL(id:allowNetwork:)` n'avait plus aucun appelant depuis que l'aperçu passe par `exportableVideoURL`, et sa documentation décrivait un refus qui n'existe plus.
5. **§64 dans le lecteur d'aperçu** (finding 9). « Aucun plan prêt à prévisualiser. Remplissez au moins une case. » persistait alors que toutes les cases peuvent être REMPLIES mais en téléchargement (§44), indisponibles (§64) ou trop courtes (§43). `PreviewPlayerView` dérive la cause de l'instantané et affiche le geste correspondant, via la table UNIQUE d'`ExportSummaryLogic` (`nothingReadyHint(_:action:)`, où `action` ne change que le but nommé). Les descriptions génériques de `PreviewError.emptyScope` et `ExportError.emptyPrefix` constatent d'abord (« Aucun plan n'est encore prêt »), puis nomment les trois gestes possibles.
6. **Vocabulaire §51** (finding 19) : `AppEnvironment` décrivait encore l'exportateur comme « §51 préfixe ».
7. **Export tronqué après réouverture de l'écran** (finding 2). L'annonce ne vivait que dans l'instance de vue qui avait appuyé sur « Exporter » : fermer puis rouvrir le résumé pendant l'encodage la perdait, et un export TRONQUÉ §66 redevenait « Montage prêt ». `ExportActor` la fige au démarrage (`announcedMontage(projectID:)`, dérivée de la timeline réellement encodée) ; `finish()` et `restoreLastOutcome()` s'y rabattent.

**Ce qui reste à faire :** **vérification Mac (⌘B/⌘U)** — le projet est généré sous Windows et n'a jamais été compilé ; aucun des tests ci-dessus n'a été exécuté. À contrôler en priorité : le rendu des **crochets multiples** de la mini-timeline sur un projet à zones nombreuses (§35.3/§82, correctif de dérive), l'**écoute d'une jonction** entre deux zones dans l'aperçu §47.2 (c'est là que le saut de musique s'entend), et le parcours §66 d'un export **tronqué** (rush rendu indisponible pendant l'encodage : le message doit dire ce qui a été écrit et pourquoi).
